...
class BinanceFuturesTestnetProtectiveOrdersEngine:
    ...
    def _validate_config(self, config: BinanceFuturesTestnetProtectiveOrdersConfig, expected_profile: str, issues: list[BinanceFuturesTestnetProtectiveIssue]) -> None:
        ...
        self._expect(0 < float(config.maximum_position_abs_quantity) <= 0.001, issues, "maximum_position_abs_quantity", "Maximum position quantity must be > 0 and <= 0.001.")
        self._expect(0 < float(config.maximum_position_notional_usdt) <= 100.0, issues, "maximum_position_notional_usdt", "Maximum notional must be > 0 and <= 100.")
