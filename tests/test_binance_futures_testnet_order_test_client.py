...
def test_exchange_filters_are_parsed_and_used() -> None:
    ...
    assert filters.price_tick_size == 0.1
    assert filters.min_qty == 0.001
    assert filters.market_min_qty == 0.001
    assert filters.raw_response_included is False
    assert client.build_order_test_preview("smcbot-test-filter-001", "BUY", "LIMIT", 0.001, price=50000, time_in_force="GTC", exchange_filters=filters).exchange_filters_valid is True
...
def test_submit_test_order_uses_only_order_test_post_and_redacts_metadata() -> None:
    ...
    preview = client.build_order_test_preview("smcbot-test-post-001", "BUY", "MARKET", 0.001)
...
def test_final_host_is_validated() -> None:
    ...
    preview = client.build_order_test_preview("smcbot-test-host-001", "BUY", "MARKET", 0.001)
