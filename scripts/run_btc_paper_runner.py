...
from models.binance_futures_testnet_order_lifecycle import BinanceFuturesTestnetLifecycleIssue
...
from reporting.binance_futures_testnet_order_lifecycle_report import format_binance_futures_testnet_order_lifecycle_result
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.analyze_futures_risk_dry_run:
        ...
        if status.state != "RUNNING":
            lifecycle_result.issues.append(
                BinanceFuturesTestnetLifecycleIssue(
                    name="runner_not_running",
                    severity="WARNING",
                    message="Runner is not running; Binance futures Testnet manual post-only order lifecycle validation executed as standalone local dry-run diagnostic.",
                    details={"runner_state": status.state},
                )
            )
        ...
        transition = None
        payload = {"runner_status": status.to_dict(), "binance_futures_testnet_order_lifecycle": lifecycle_result.to_dict()}
        rendered = format_btc_paper_runner_status_report(status, transition) + "\n\n" + format_binance_futures_testnet_order_lifecycle_result(lifecycle_result)
    ...
    if (
        ...
        or args.validate_binance_futures_testnet_order_lifecycle_dry_run
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--validate-binance-futures-testnet-order-lifecycle-dry-run", action="store_true")
